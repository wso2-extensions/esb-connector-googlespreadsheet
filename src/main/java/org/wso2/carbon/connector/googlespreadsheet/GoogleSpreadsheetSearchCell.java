...
public class GoogleSpreadsheetSearchCell extends AbstractConnector {
    ...
    public void connect(MessageContext messageContext) throws ConnectException {
        try {
            ...
            String searchString = GoogleSpreadsheetUtils
                    .lookupFunctionParam(messageContext, SEARCH_STRING);
            if (worksheetName == null || "".equals(worksheetName.trim())
                    || spreadsheetName == null
                    || "".equals(spreadsheetName.trim())
                    || searchString == null || "".equals(searchString.trim())) {
                log.error("Please make sure you have given a valid input for the worksheet, spreadsheet and search string");
                ConnectException connectException = new ConnectException("Please make sure you have given a valid input for the worksheet, spreadsheet and search string");
            ...
            List<String> resultData = gssData.search(wsEntry, searchString);
            ...
            OMElement searchResult = factory.createOMElement("searchCellResult", ns);
            ...
            for (int iterateCount = 0; iterateCount < resultSize; iterateCount++) {
                if (resultData.get(iterateCount) != null) {
                    OMElement cellId = factory.createOMElement("cellId", ns);
                    data.addChild(cellId);
                    cellId.setText(resultData.get(iterateCount));
